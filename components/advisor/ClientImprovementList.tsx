...
import { TrendingUp, Users, ArrowRight, ChevronDown, ChevronUp, Target, Zap } from 'lucide-react';
...
import type { AdvisorWithProfile, ClientNeed } from '@/types/database';

interface ClientImprovementListProps {
...
function calculateSkillImprovementPotential(
  ...
  // Create a modified skills array with the improved skill
...
export function ClientImprovementList({ advisor }: ClientImprovementListProps) {
  const [expandedClient, setExpandedClient] = useState<string | null>(null);
  
  const clientBook = useMemo(() => getAdvisorClientBook(advisor.id), [advisor.id]);

  const clientImprovements = useMemo(() => {
    return clientBook.map(({ client, needs, fitScore, annualFee }) => {
      ...
      needs.forEach(need => {
        ...
        // Only consider skills where advisor is under-qualified
        if (currentLevel < neededLevel) {
          const potentialImprovement = calculateSkillImprovementPotential(
            advisor,
            needs,
            need.subtopic_id,
            neededLevel // Assume improving to needed level
          );
      ...
      // Sort by potential improvement (transfer improvement + skill improvement)
  ...
  // Summary stats
  const stats = useMemo(() => {
    const transferable = clientImprovements.filter(c => c.betterAdvisor && c.betterAdvisor.improvement >= 10);
    const improvableBySkill = clientImprovements.filter(c => c.topSkillGap && c.topSkillGap.potentialImprovement >= 5);
    const avgScore = clientImprovements.length > 0
      ? Math.round(clientImprovements.reduce((sum, c) => sum + c.currentFitScore, 0) / clientImprovements.length)
      : 0;
    
    return {
      total: clientImprovements.length,
      avgScore,
        ...
        <Card className="bg-muted/30">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              ...
              <ArrowRight className="h-8 w-8 text-warning opacity-70" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-muted/30">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              ...
              <Zap className="h-8 w-8 text-success opacity-70" />
            </div>
          </CardContent>
        </Card>
      </div>

      {/* Client Improvement Table */}
      <Card>
                              ...
                              <div className="p-4 rounded-lg border bg-background">
                                ...
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">{client.betterAdvisor.name}</span>
                                  </div>
                                  <div className="flex justify-between">
                            ...
                            {client.topSkillGap && (
                              <div className="p-4 rounded-lg border bg-background">
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <h4 className="font-semibold">Develop Skill to Improve Fit</h4>
                                </div>
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
